...
async function resolveColumnId(userId: string, col: string | number): Promise<string | null> {
  ...
  if (/^col\d+$/i.test(target)) {
    ...
    const { rows } = await pool.query(
      `SELECT id FROM public.leads WHERE user_id = $1 ORDER BY id ASC`,
      [userId]
    )
    if (rows[idx]) {
  ...
  if (/^\d+$/.test(target)) {
    const { rows } = await pool.query(
      `SELECT id FROM public.leads WHERE user_id = $1 AND id = $2`,
      [userId, target]
    )
    if (rows.length > 0) {
