-- FUNCIÓN CORREGIDA SIN AMBIGÜEDAD
...
DECLARE
  ...
  trig_name TEXT;
BEGIN
  -- DESHABILITAR TODOS LOS TRIGGERS TEMPORALMENTE (con alias para evitar ambigüedad)
  SELECT array_agg(t.trigger_name) INTO current_triggers
  FROM information_schema.triggers t
  WHERE t.event_object_table = 'personalities';
  
  IF current_triggers IS NOT NULL THEN
    FOREACH trig_name IN ARRAY current_triggers
    LOOP
      EXECUTE format('ALTER TABLE personalities DISABLE TRIGGER %I', trig_name);
    END LOOP;
  END IF;
  
  -- INSERTAR SIN TRIGGERS
  ...
  IF current_triggers IS NOT NULL THEN
    FOREACH trig_name IN ARRAY current_triggers
    LOOP
      EXECUTE format('ALTER TABLE personalities ENABLE TRIGGER %I', trig_name);
    END LOOP;
  END IF;
