-- =========================================================
-- Identificar y consolidar conversaciones duplicadas
-- =========================================================
...
-- 3. CONSOLIDAR CONVERSACIONES DUPLICADAS
-- IMPORTANTE: Ejecutar esto con cuidado, hacer backup primero
-- Este script mantiene la conversación más reciente y mueve los mensajes a ella
...
BEGIN
    ...
    FOR dup_record IN 
        ...
        HAVING COUNT(*) > 1
    LOOP
        -- Seleccionar la conversación principal (la que tiene más mensajes o es más reciente)
        SELECT c.id INTO main_conv_id
        FROM conversations_new c
        WHERE c.external_id = dup_record.external_id
          AND c.user_id = dup_record.user_id
        ORDER BY 
            (SELECT COUNT(*) FROM messages_new WHERE conversation_id = c.id) DESC,
            c.updated_at DESC
        LIMIT 1;
        
        IF main_conv_id IS NULL THEN
            CONTINUE; -- Saltar si no se encontró conversación principal
        END IF;
        
        RAISE NOTICE 'Consolidando conversaciones para % - Conversación principal: %', 
            dup_record.external_id, main_conv_id;
        ...
        LOOP
            ...
            SET 
                contact_name = COALESCE(
                    (SELECT contact_name FROM conversations_new WHERE id = dup_conv_id AND contact_name IS NOT NULL),
                    contact_name
                ),
                contact_photo_url = COALESCE(
                    (SELECT contact_photo_url FROM conversations_new WHERE id = dup_conv_id AND contact_photo_url IS NOT NULL),
                    contact_photo_url
                ),
                updated_at = GREATEST(updated_at, (SELECT updated_at FROM conversations_new WHERE id = dup_conv_id))
