...
export async function POST(req) {
    ...
    try {
        const { rows } = await pool.query(`select id, name, phone
         from public.leads_contacts
        where user_id=$1 and conversation_id is null and phone is not null`, [userId]);
        ...
        for (const r of rows) {
            try {
                ...
                // Upsert conversación
                const upd = await pool.query(`update public.conversations_new
              set contact_name=$3, updated_at=now(), tenant='whatsapp'
            where user_id=$1 and external_id=$2`, [userId, jid, contactName]);
                if (upd.rowCount === 0) {
                    await pool.query(`insert into public.conversations_new
              (user_id, external_id, contact_name, contact_photo_url, started_at, ai_active, tenant, updated_at, wa_user_id)
             values ($1,$2,$3,null, now(), false, 'whatsapp', now(), null)`, [userId, jid, contactName]);
                    created++;
                }
                // Actualiza el lead con su JID
                await pool.query(`update public.leads_contacts
              set conversation_id=$1, updated_at=now()
            where id=$2 and user_id=$3`, [jid, r.id, userId]);
                updated++;
            }
            catch (e) {
