...
export async function POST(req) {
    ...
    try {
        ...
        const { rows: convs } = await pool.query(`select external_id, contact_name, contact_photo_url
         from public.conversations_new
        where user_id = $1
          and external_id is not null
          and external_id not like '%@g.us'`, [userId]);
        ...
        for (const c of convs) {
            ...
            const exist = await pool.query(`select 1 from public.leads_contacts where user_id=$1 and conversation_id=$2 limit 1`, [userId, jid]);
