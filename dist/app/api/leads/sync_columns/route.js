...
export async function POST(req) {
    ...
    try {
        ...
        const existing = await pool.query(`select id, title, color from public.leads where user_id=$1 order by id asc`, [userId]);
        const result = [];
        for (const c of columns) {
           ...
           values ($1,$2,$3, now(), now()) returning id`, [userId, title, color]);
