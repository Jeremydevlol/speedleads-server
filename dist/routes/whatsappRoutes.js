...
import { disconnectSession, isSessionConnected } from '../services/whatsappService.js'
...
router.get('/get_contacts', validateJwt, async (req, res) => {
  try {
    ...
    console.log(`📞 [GET /get_contacts] Solicitado por userId=${userId}`);
    
    const contacts = await whatsappController.getContacts(userId)
    
    console.log(`✅ [GET /get_contacts] Retornando ${contacts.length} contactos para userId=${userId}`);
    ...
    console.error('❌ [GET /get_contacts] Error obteniendo contactos:', err);
...
router.get('/status', validateJwt, async (req, res) => {
  try {
    ...
    return res.json({ 
      success: true, 
      connected,
      message: connected ? 'WhatsApp conectado' : 'WhatsApp no conectado',
      needsQr: !connected
    })
  } catch (err) {
    ...
    return res.status(500).json({ 
      success: false, 
      message: err.message || 'Error verificando estado',
      connected: false
    })
  }
})
...
router.post('/disconnect', validateJwt, async (req, res) => {
  try {
    ...
    console.log(`🔌 Solicitud de desconexión WhatsApp para userId=${userId}`);
    ...
    return res.status(500).json({ 
      success: false, 
      message: err.message || 'Error al desconectar WhatsApp'
    })
  }
})
...
/**
 * POST /api/whatsapp/react_to_message
 * Reaccionar a un mensaje de WhatsApp
 * Body: { messageId: number, emoji: string (opcional, "" para quitar) }
 */
router.post('/react_to_message', validateJwt, whatsappController.reactToMessageController)
