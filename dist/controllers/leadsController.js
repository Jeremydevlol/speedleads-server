...
export async function moveLead(req, res) {
  try {
    ...
    // Validar que la columna destino existe y pertenece al usuario
    const columnCheck = await pool.query(
      `SELECT id FROM public.leads WHERE id = $1 AND user_id = $2`,
      [targetColumnId, userId]
    );

    if (!columnCheck.rows.length) {
      console.log(`⚠️ Columna destino no encontrada o no pertenece al usuario: ${targetColumnId} para userId=${userId}`);
      return res.status(400).json({ 
        success: false, 
        message: "Columna destino no válida o no pertenece al usuario" 
      });
    }

    // Verificar que el lead existe y pertenece al usuario antes de moverlo
    const leadCheck = await pool.query(
      `SELECT id, column_id FROM public.leads_contacts WHERE id = $1 AND user_id = $2`,
      [leadId, userId]
    );
    ...
    if (!leadCheck.rows.length) {
      return res.status(404).json({ 
        success: false, 
        message: "Lead no encontrado o no pertenece al usuario" 
      });
    }

    // Si el lead ya está en la columna destino, no hacer nada
    if (leadCheck.rows[0].column_id === targetColumnId) {
      return res.json({ 
        success: true, 
        lead: leadCheck.rows[0],
        message: "El lead ya está en esta columna" 
      });
    }
    ...
    const { rows } = await pool.query(
      `UPDATE public.leads_contacts
       ...
       RETURNING id, name, column_id, conversation_id, updated_at`,
    ...
    console.log(`✅ Lead ${leadId} movido a columna ${targetColumnId} para userId=${userId}`);
