...
async function linkUserToStripeCustomer(userId, stripeCustomerId, email) {
    await supabase
        ...
        .upsert({ user_id: userId, email: email ?? null, stripe_customer_id: stripeCustomerId, updated_at: new Date().toISOString() }, { onConflict: 'user_id' });
}
async function upsertSubscriptionFromStripe(sub, userId) {
...
async function recordInvoiceFromStripe(inv) {
