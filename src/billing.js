...
router.get('/plans', (req, res) => {
  const makePlan = (id, name, amount, linkEnv) => {
    const link = (linkEnv ?? '').trim() || null; // no fallbacks tipo /test-*
    return {
      id,
      name,
      amount_cents: amount,
      amountCents: amount,        // compat camel
      interval: 'month',          // todos mensuales
      currency: 'eur',
      payment_link: link,
      paymentLink: link,          // compat camel
    };
  };
  ...
  ].filter(p => !!p.payment_link); // solo los que tengan link real
